...
export interface SimSettings {
    bodyCount: number
    radiusMin: number      // meters
    radiusMax: number      // meters
    massMin: number        // kg
    massMax: number        // kg
    velocityMode: VelocityMode
    velocityScale: number  // multiplier for initial velocity
...
export type GravityChangeCallback = (gravityType: GravityType) => void

export function createSettingsPanel(
    onApply: SettingsChangeCallback,
    onGravityChange: GravityChangeCallback
): HTMLElement {
    ...
    const velocityModeSelect = panel.querySelector('#set-velocityMode') as HTMLSelectElement
    const modeDesc = panel.querySelector('#velocity-mode-desc')
    const velocityScaleField = panel.querySelector('#velocity-scale-field') as HTMLElement

    const modeDescriptions: Record<VelocityMode, string> = {
    ...
    if (velocityModeSelect && modeDesc) {
        velocityModeSelect.addEventListener('change', () => {
            const mode = velocityModeSelect.value as VelocityMode
    ...
    const velocityScaleSlider = panel.querySelector('#set-velocityScale') as HTMLInputElement
...
export function getSettingsFromPanel(): SimSettings {
    ...
    const getSelect = (id: string, fallback: string) => {
    ...
    return {
        ...
        velocityMode: getSelect('set-velocityMode', DEFAULT_SETTINGS.velocityMode) as VelocityMode,
...
export function updateSettingsPanelValues(settings: SimSettings): void {
...
export function setGravityAlgoValue(gravityType: GravityType): void {
    const select = document.getElementById('set-gravityAlgo') as HTMLSelectElement
    if (select) select.value = gravityType
}
