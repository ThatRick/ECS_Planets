...
export const GravitySystem: System = {
    ...
    update(world: World, dt: number): void {
        ...
        const mergedEntities = new Set<number>()
        const pairs = spatialHash.getPotentialPairs()

        for (const [idA, idB] of pairs) {
            // Skip if either entity already merged
            if (mergedEntities.has(idA) || mergedEntities.has(idB)) continue
            ...
            if (dist < sizeA + sizeB) {
                ...
                const combinedVel = Vec2.scale(
                    Vec2.add(
                        Vec2.scale(winnerVel, winnerMass),
                        Vec2.scale(loserVel, loserMass)
                    ),
                    1 / combinedMass
                )

                // Center of mass position
                const combinedPos = Vec2.scale(
                    Vec2.add(
                        Vec2.scale(winnerPos, winnerMass),
                        Vec2.scale(loserPos, loserMass)
                    ),
                    1 / combinedMass
                )
                ...
                console.log(
                    `Merge: M=${combinedMass.toExponential(1)}kg, ` +
        ...
        const accelerations = new Map<number, Vec2>()
