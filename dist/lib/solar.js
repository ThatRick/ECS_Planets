 ...
 */
export function greenwichSiderealAngleRad(utcMs) {
    const JD = utcMs / 86_400_000 + 2_440_587.5;
    const T = (JD - 2_451_545.0) / 36525;
 ...
 */
export function inertialToEarthFixedWorld(utcMs, inX, inY, inZ, out) {
 ...
 */
export function earthFixedToInertialWorld(utcMs, inX, inY, inZ, out) {
    const theta = greenwichSiderealAngleRad(utcMs);
    const c = Math.cos(theta);
    const s = Math.sin(theta);
    out[0] = c * inX - s * inZ;
...
export function isInEarthShadow(satX, satY, satZ, sunDirX, sunDirY, sunDirZ, earthRadius) {
    return earthShadowSignedDistance(satX, satY, satZ, sunDirX, sunDirY, sunDirZ, earthRadius) < 0;
}
/**
 * Signed distance (meters) to the boundary of the cylindrical Earth shadow model.
 * Negative = inside shadow, positive = sunlit side.
 */
export function earthShadowSignedDistance(satX, satY, satZ, sunDirX, sunDirY, sunDirZ, earthRadius) {
 ...
 */
export function sunlitByteFromEarthShadow(satX, satY, satZ, sunDirX, sunDirY, sunDirZ, earthRadius, transitionDistanceMeters) {
    const signedDist = earthShadowSignedDistance(satX, satY, satZ, sunDirX, sunDirY, sunDirZ, earthRadius);
