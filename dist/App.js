...
export default class App {
     ...
     */
    calculateVelocity(pos, r, mode, scale, totalMass) {
        switch (mode) {
            case 'orbital': {
                ...
                return new Vec3(-Math.sin(angle) * orbitalSpeed, 0, // No Y velocity for disc
            ...
            case 'collapse': {
                ...
                return new Vec3((Math.random() - 0.5) * maxSpeed, (Math.random() - 0.5) * maxSpeed, (Math.random() - 0.5) * maxSpeed);
            }
            case 'static':
            default: {
                // Small random velocities for 3D point cloud
                const smallSpeed = scale * 10; // Small random velocities
                return new Vec3((Math.random() - 0.5) * smallSpeed, (Math.random() - 0.5) * smallSpeed, (Math.random() - 0.5) * smallSpeed);
            }
        }
    }
    /**
     ...
     */
    generatePosition(radiusMin, radiusMax, mode) {
        ...
        if (mode === 'orbital') {
            ...
            return new Vec3(Math.cos(angle) * r, (Math.random() - 0.5) * discThickness, Math.sin(angle) * r);
        }
        else {
            ...
            return new Vec3((x / len) * r, (y / len) * r, (z / len) * r);
        }
    }
    resetSimulation(settings) {
        ...
        for (let i = 0; i < settings.bodyCount; i++) {
            ...
            const pos = this.generatePosition(settings.radiusMin, settings.radiusMax, settings.velocityMode);
            const r = pos.len();
            // Calculate velocity based on mode
            const vel = this.calculateVelocity(pos, r, settings.velocityMode, settings.velocityScale, totalMass);
            // Random mass
    ...
    updateLegendStatusCounts() {
        ...
        const countEls = this.legendPanel.content.querySelectorAll('[data-status-count]');
        countEls.forEach((el) => {
    ...
    setLegendStatusVisibilityForAll(visible) {
        if (!this.legendPanel)
            return;
        const checkboxes = this.legendPanel.content.querySelectorAll('input[type="checkbox"][data-status]');
    ...
    selectStarlinkSatellite(entityId) {
        const renderer = this.sharedRendererSystem;
        this.selectedEntity = entityId;
        if (renderer)
            renderer.selectedEntity = entityId;
        this.updateInfoPanel(entityId);
    }
    findExtremeAltitudeStarlinkEntity(selectHighest) {
        let match;
    ...
    findSatelliteByNoradId(noradId) {
    ...
    selectHighestStarlinkSatellite() {
        if (this.currentScene !== 'starlinks')
            return;
    ...
    updateInfoPanel(entityId) {
        ...
        if (renderer?.sunlightMode) {
            ...
            if (pos) {
                ...
                const shadowed = isInEarthShadow(pos.x, pos.y, pos.z, sunDir[0], sunDir[1], sunDir[2], EARTH_RADIUS_M);
    ...
    setupProtoPlanets(world) {
        ...
        for (let i = 0; i < config.bodyCount; i++) {
            ...
            // Generate 3D position based on mode
            const pos = this.generatePosition(config.radiusMin, config.radiusMax, config.velocityMode);
            const r = pos.len();
            // Calculate velocity based on mode
            const vel = this.calculateVelocity(pos, r, config.velocityMode, config.velocityScale, totalMass);
            // Random mass
...
function parseStarlinkOrbit(rec, muEarth) {
...
function parseTleEpochMs(epoch) {
    // YYDDD.DDDDDDDD
    if (epoch.length < 5)
        return NaN;
...
function orbitToComponent(orbit) {
...
function setOrbitTimeMs(orbit, simTimeMs) {
...
function setPositionFromOrbit(pos, orbit) {
